...
def heuristic_search(algorithm, sort_by):
    ...
    while len(frontier) > 0:
        ...
        # We need to remove the current node from the frontier and add it to the visited.
    ...
    if goal_state is None:  # Check if goal_state is not None
        ...
        while current_node is not None:
            ...
            maze_ins = maze.Maze()
            solution.append(maze_ins.coordinate_from_index(current_node.square_number))
...
def add_to_frontier(current_node, algorithm):
    ...
    nodes_to_add = []
    current_node.direction(current_node.square_number)
...
def is_goal(node):
    return node.square_number in graph.maze.goal_squares
...
def print_results(algorithm, solution_cost, solution, expanded_nodes):
    ...
    for node in solution:
        print(node, end=" ")  # Print square_id instead of x and y
    ...
    if "IDS" in algorithm:
        ...
        for node in expanded_nodes:
            print(node.square_number, end=" ")  # Print square_id instead of x and y
    print("\n")
